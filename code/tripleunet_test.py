...
def get_accuracy(model, single_unet, data):
    ...
    with torch.no_grad():
        for imgs, labels in data:
            ...
            # preds0 = torch.sigmoid(preds[0])
            # # preds = torch.sigmoid(model(imgs))
            # preds0 = (preds0 > 0.5).float()

            # preds1 = torch.sigmoid(preds[1])
            # # preds = torch.sigmoid(model(imgs))
            # preds1 = (preds1 > 0.5).float()
            # preds = torch.sigmoid(preds0+ preds1)
            # # print(preds.shape)
            ...
            predictions.append(preds.repeat(3, 1, 1))


            ######## triple_unet #########

            preds3 = torch.sigmoid(single_unet.forward(imgs))
            preds3 = (preds3 > 0.5).float()
            preds3 = preds3.squeeze(0)
            
            preds = torch.logical_or(preds, preds3)
            # could use various other methods such as logical_and, concatenation, 
            # weighted algorithms, etc. but logical_or was the best out of all we tried
            preds = preds.squeeze(0)
...
if __name__ == "__main__":
    ...
    f = open("tripleunet_analysis.txt", "w")
