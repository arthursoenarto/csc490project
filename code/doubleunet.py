...
# code mostly from https://arxiv.org/abs/2006.04868 paper
...
class decoder1(nn.Module):
    ...
    def forward(self, x, skip):
        ...
        ones = torch.zeros(x.shape[0], x.shape[1], 1, 1)
        # print(ones.shape)
        # x = torch.stack([x, ones], 1)
        # x = torch.vstack((x, ones))
        curr_total = x.shape[0] * x.shape[1] * x.shape[2] * x.shape[3]
        shape0 = x.shape[0]
        shape1 = x.shape[1]
        shape2 = x.shape[2] + 1
        shape3 = x.shape[3] + 1
        new_total = shape0 * shape1 * shape2 * shape3
        x = torch.reshape(x, (-1,))
        # print(curr_total, new_total)
        zeros = torch.zeros(new_total-curr_total)
        x = torch.cat([x, zeros])
        # x = torch.reshape(x, (x.shape[0], x.shape[1], x.shape[2]+1, x.shape[3]+1))
        x = torch.reshape(x, (shape0, shape1, shape2, shape3))
        # print(x.shape)
        # print(s1.shape)
        ...
        shape3 = x.shape[3] + 1
        new_total = shape0 * shape1 * shape2 * shape3
        # print(curr_total, new_total, new_total - curr_total)
        x = torch.reshape(x, (-1,))
        zeros = torch.zeros(new_total - curr_total)
        x = torch.cat([x, zeros])
        # x = torch.reshape(x, (x.shape[0], x.shape[1], x.shape[2]+1, x.shape[3]+1))
...
class decoder2(nn.Module):
    ...
    def forward(self, x, skip1, skip2):
        ...
        x = self.up(x)
        # print(x.shape, skip1[0].shape, skip2[0].shape)
        # print(skip1[1].shape, skip2[1].shape)
        # print(skip1[2].shape, skip2[2].shape)
        # print(skip1[3].shape, skip2[3].shape)
        curr_total = x.shape[0] * x.shape[1] * x.shape[2] * x.shape[3]
        shape0 = x.shape[0]
        shape1 = x.shape[1]
        shape2 = x.shape[2] + 1
